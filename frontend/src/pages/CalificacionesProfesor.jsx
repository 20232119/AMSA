...
export default function CalificacionesProfesor() {
  ...
  async function reopenGrades() {
    ...
    try {
      ...
      await Swal.fire({
        icon: 'success',
        title: 'Notas reabiertas',
        text: 'Puedes corregir las calificaciones y volver a enviarlas.',
      ...
      await Swal.fire({
        ...
        text: e.message || 'Ocurrió un error al reabrir las notas.',
  ...
  async function submitAll() {
    ...
    try {
      ...
      await Swal.fire({
        icon: 'success',
        title: 'Enviado a Registro',
        text: 'Las notas fueron enviadas correctamente para validación.',
        confirmButtonText: 'Aceptar',
      })
    } catch (e) {
      const message = e.message || 'No se pudo enviar la sección.'
      ...
      const isAttendanceError =
        ...
        lower.includes('sesiones de asistencia') ||
        lower.includes('falta completar la asistencia')

      await Swal.fire({
        icon: 'warning',
        title: isAttendanceError
          ? 'No se puede enviar sin asistencia'
          : 'No se pudo enviar',
        text: message,
        confirmButtonText: 'Entendido',
      })
  ...
  const allEntered = rows.length > 0 && rows
    .filter(r => r.enrollmentStatus !== 'retirado')
    .every(r => {
                ...
                {rows.map((row, i) => {
                  ...
                  return (
                    <tr
                      key={row.student.id}
                      style={{
                        background: row.enrollmentStatus === 'retirado' ? '#FFF8F0' : bg,
                        opacity: row.enrollmentStatus === 'retirado' ? .7 : 1
                      }}
                    >
