...
export default function CalificacionesRegistro() {
  ...
  async function validate(gradeId) {
    ...
    setMsg({ type:'success', text:'Nota validada.' })
    await loadGrades()
  }

  async function reject(gradeId) {
    await api.patch(`/grades/${gradeId}/reject`, {})
    setMsg({ type:'success', text:'Nota devuelta al profesor.' })
    await loadGrades()
  }

  async function rejectAll() {
    await api.post(`/grades/section/${activeId}/reject-all`, {})
    setMsg({ type:'success', text:'Todas las notas devueltas al profesor.' })
    await loadGrades()
  }

  async function publishAll() {
    await api.post(`/grades/section/${activeId}/publish`, {})
    setMsg({ type:'success', text:'¡Notas publicadas!' })
    await loadGrades()
  }
  ...
  async function toggleEnrollment(enrollmentId, currentStatus) {
    ...
    setMsg({ type:'success', text: newStatus === 'retirado' ? 'Estudiante marcado como retirado.' : 'Estudiante reactivado.' })
    await loadGrades()
  }
  ...
  async function toggleManualPresent() {
    ...
    setMsg({ type:'success', text: res.allowManualPresent ? 'Permiso de Presente manual activado para el profesor.' : 'Permiso de Presente manual desactivado.' })
  }
    ...
    <AppShell navItems={navItems}>
      ...
      {loading ? <PageSpinner /> : rows.length === 0 ? (
        ...
        <Card>
          ...
          {/* Warning if manual present is on */}
          {allowManual && (
