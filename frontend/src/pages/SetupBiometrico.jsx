  ...
  async function handleRegister() {
    ...
    try {
      ...
      if (!optionsJSON.challenge) {
        throw new Error('Las opciones de registro no contienen challenge.')
      }

      if (!optionsJSON.user?.id) {
        throw new Error('Las opciones de registro no contienen un user.id válido.')
      }
      ...
      await api.post('/auth/webauthn/register/verify', {
        ...attResp,
        friendlyName: `Dispositivo ${creds.length + 1}`,
      })
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 600 }}>
                      {c.friendlyName ?? 'Dispositivo'}
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--stone-400)' }}>
                      {c.deviceType ?? 'Desconocido'} · Último uso:{' '}
                      {c.lastUsedAt
                        ? new Date(c.lastUsedAt).toLocaleDateString('es-DO')
                        : 'Nunca'}
                    </div>
                  </div>
