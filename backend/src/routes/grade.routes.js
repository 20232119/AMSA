...
router.post('/section/:sectionId/submit', requireRole('profesor'), async (req, res, next) => {
  try {
    ...
    const gradeMap = Object.fromEntries(grades.map(g => [g.studentId, g]))
    const totalSessions = section.classSessions.length

    if (totalSessions === 0) {
      return res.status(400).json({
        error: 'Aún no se han generado las sesiones de asistencia para esta sección.',
      })
    }

    const missingAttendance = active.filter(enr => enr.attendance.length < totalSessions)

    if (missingAttendance.length > 0) {
      return res.status(400).json({
        error: `Falta completar la asistencia de: ${missingAttendance.map(e => `${e.student.firstName} ${e.student.lastName}`).join(', ')}`,
        missingAttendance: missingAttendance.map(e => e.studentId),
      })
    }

    const missingGrades = active.filter(enr => {
    ...
    if (missingGrades.length > 0) {
      return res.status(400).json({
        error: `Faltan notas para: ${missingGrades.map(e => `${e.student.firstName} ${e.student.lastName}`).join(', ')}`,
        missing: missingGrades.map(e => e.studentId),
      })
    }
