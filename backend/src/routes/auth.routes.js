...
router.post('/login', loginLimiter, async (req, res, next) => {
  try {
    ...
    if (!identifier || !password) return res.status(400).json({ error: 'Campos requeridos' })
    ...
    res.json({
      ...
      user: {
        id: full.id, institutionalId: full.institutionalId,
        firstName: full.firstName, lastName: full.lastName,
        email: full.email, role: full.role.name,
...
router.post('/refresh', async (req, res, next) => {
  try {
    ...
    const user        = await prisma.user.findUnique({ where: { id: token.userId }, include: { role: true } })
    const accessToken = makeAccessToken(user)
...
router.get('/me', requireAuth, async (req, res, next) => {
  try {
    ...
    res.json({
      id: user.id, institutionalId: user.institutionalId,
...
// Convert Uint8Array → base64url string
    ...
    await prisma.auditLog.create({ data: { userId: user.id, action: 'WEBAUTHN_REGISTER' } })
...
router.post('/webauthn/authenticate/verify', async (req, res, next) => {
  try {
    ...
    if (!challenge || !user) return res.status(400).json({ error: 'Challenge inválido' })

    // ← el fix va aquí
    const credId       = body.id
    const credIdString = typeof credId === 'string'
      ? credId
      : Buffer.from(credId).toString('base64url')

    const cred = await prisma.webAuthnCredential.findFirst({ 
      where: { credentialId: credIdString },
    })
    ...
    await prisma.auditLog.create({
      data: { userId: user.id, action: 'LOGIN_BIOMETRIC', ipAddress: req.ip },
    })
...
router.get('/webauthn/credentials', requireAuth, async (req, res, next) => {
  try {
    const creds = await prisma.webAuthnCredential.findMany({
      ...
      select: { id: true, friendlyName: true, deviceType: true, lastUsedAt: true, createdAt: true },
    })
    res.json(creds)
  } catch (e) { next(e) }
})
...
router.delete('/webauthn/credentials/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!cred) return res.status(404).json({ error: 'Credencial no encontrada' })
    await prisma.webAuthnCredential.update({ where: { id: cred.id }, data: { isActive: false } })
    res.json({ ok: true })
  } catch (e) { next(e) }
})
